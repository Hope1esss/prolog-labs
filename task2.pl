...
% Предикат для вычисления среднего балла и проверки, сдал ли студент
student_average(Student, Average, Passed) :-
    findall(Grade, (subject(_, Grades), member(grade(Student, Grade), Grades)), AllGrades),
    sum_list(AllGrades, Sum),
    length(AllGrades, Count),
    Count > 0,  % Убедиться, что у студента есть оценки
    Average is Sum / Count,
    (memberchk(Grade, AllGrades), Grade < 3 -> Passed = no; Passed = yes).


% Предикат для подсчёта не сдавших студентов по предмету
failed_students_per_subject(Subject, FailedCount) :-
    subject(Subject, Grades),
    findall(Student, (member(grade(Student, Grade), Grades), Grade < 3), FailedStudents),
    length(FailedStudents, FailedCount).


% Предикат для нахождения студентов с максимальным средним баллом в группе
top_students_per_group(Group, TopStudents) :-
    group(Group, Students),
    findall(Avg-Student, 
            (member(Student, Students), student_average(Student, Avg, _)), 
            Averages),
    max_member(MaxAvg-_, Averages),
    findall(Student, member(MaxAvg-Student, Averages), TopStudents).
